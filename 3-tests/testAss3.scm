...
(define runAllTests
  (lambda (_list)
    (let ((results (map (lambda (test) (runTests (car test) (cdr test))) _list)))
	(if show-summary
	  (begin
...
(define Comp161Ass3Tests
  (list
    ...
    '(define make-sq-jump (lambda (_list)
                      (letrec ((main-func (lambda (make-sq-jump res-_list)
											(lambda (a)
											 (set! make-sq-jump make-sq-jump)
											 (set! res-_list res-_list)
											 (if (null? (cdr (memq a _list)))
                                             (list _list)
                                             (let ((res-_list (make-sq-jump (cdr (memq a _list)))))
                                               (append (list (append (list a )
                                                                       (filter (lambda (res-_list-element)
                                                                                 ( > (car res-_list-element) (* a a)))
                                                                               res-_list )))
                                                       res-_list )))))))
                             (main-func  (car _list)))))
